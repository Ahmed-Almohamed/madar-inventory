...
function setup(){
 const db=new DatabaseSync(':memory:');db.exec('PRAGMA foreign_keys=ON');db.exec(firstMigration);db.exec(secondMigration);db.exec(readFileSync(new URL('../migrations/0003_warehouse_details.sql',import.meta.url),'utf8'));db.exec(readFileSync(new URL('../migrations/0004_product_images.sql',import.meta.url),'utf8'));db.exec(readFileSync(new URL('../migrations/0005_shipping_fee.sql',import.meta.url),'utf8'));
